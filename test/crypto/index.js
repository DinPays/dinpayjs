...
describe("different networks", function () {
  ...
  it("validate address on tesnet should be ok", function () {
    DinPay.crypto.setNetworkVersion(0x52);
    DinPay.crypto.getNetworkVersion().should.equal(0x52);
    var validate = DinPay.crypto.validateAddress("a6fpb1BJZq4otWiVsBcuLG1ZGs5WsqqQtH");
    (validate).should.equal(true);
    DinPay.crypto.setNetworkVersion(0x17);
    DinPay.crypto.getNetworkVersion().should.equal(0x17);
  });
});

describe("delegate.js", function () {
  var delegate = DinPay.delegate;
  ...
  describe("#createDelegate", function () {
    ...
    describe("returned delegate", function () {
      var keys = DinPay.crypto.getKeys("secret");
      var secondKeys = DinPay.crypto.getKeys("secret 2");
      ...
      it("should be second signed correctly", function () {
        var result = DinPay.crypto.verifySecondSignature(trs, secondKeys.publicKey);
        (result).should.be.ok;
      });
