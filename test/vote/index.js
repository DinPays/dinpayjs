...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = DinPay.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      ...
      var secondSecretKey = DinPay.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = DinPay.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.DinPay);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = DinPay.crypto.fromBytes(DinPay.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(DinPay.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = DinPay.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = DinPay.crypto.verifySecondSignature(vt, DinPay.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
