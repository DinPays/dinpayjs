...
describe("transaction.js", function () {
  ...
  var transaction = DinPay.transaction;
  ...
  describe("#createTransaction", function () {
    ...
    it("should create transaction without second signature from keys", function () {
      var secretKey = DinPay.ECPair.fromSeed("secret");
    ...
    describe("returned transaction", function () {
      ...
      it("should be signed correctly", function () {
        var result = DinPay.crypto.verify(trs);
        result.should.equal(true);
      });

      it("should be deserialised correctly", function () {
        var deserialisedTx = DinPay.crypto.fromBytes(DinPay.crypto.getBytes(trs).toString("hex"));
  ...
  describe("createTransaction and try to tamper signature", function(){
    ...
    it("should not validate overflown signatures", function(){
      ...
      var old_transaction = DinPay.transaction.createTransaction('AacRfTLtxAkR3Mind1XdPCddj1uDkHtwzD', 1, null, 'randomstring');
      ...
      */
      ...
      // console.log("TXID " + DinPay.crypto.getId(old_transaction));
      // console.log("VERIFY " + DinPay.crypto.verify(old_transaction));
      // console.log("SIG " + old_transaction.signature + "\n");

      DinPay.crypto.verify(old_transaction).should.equal(true);
      ...
      DinPay.crypto.verify(old_transaction).should.equal(false);
  ...
  describe("#createTransaction with second secret", function () {
    ...
    var keys = DinPay.crypto.getKeys(secondSecret);
    ...
    describe("returned transaction", function () {
      ...
      it("should be second signed correctly", function () {
        var result = DinPay.crypto.verifySecondSignature(trs, keys.publicKey);
        (result).should.equal(true);
      });
